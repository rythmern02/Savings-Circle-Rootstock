...
import { useCreatePot, useTotalPots } from "@/hooks/useSavingsContract"
...
export default function CreatePoolPage() {
  ...
  const { createPot, hash, isPending, error } = useCreatePot()
  const { data: receipt, isLoading: isConfirming, isSuccess: isConfirmed } = useWaitForTransactionReceipt({
    hash,
  })
  const { data: totalPotsBefore } = useTotalPots()
  ...
  const handleSubmit = async () => {
    ...
    if (!isConnected) {
      setErrors({ ...errors, agreeToTerms: "Please connect your wallet first" })
      return
    }

    try {
      // Convert months to seconds for cycle duration
      const durationMonths = parseInt(formData.duration)
      const cycleDurationSeconds = BigInt(durationMonths * 30 * 24 * 60 * 60) // Approximate seconds in a month
      
      // Calculate total cycles - this would be based on your pool logic
      // For now, we'll use duration as cycles
      const totalCycles = BigInt(durationMonths)
      
      await createPot(
        formData.poolName,
        formData.description,
        formData.monthlyContribution, // Contribution amount in RBTC
        totalCycles,
        cycleDurationSeconds
      )
      
      // Note: The actual redirect happens after transaction confirmation via useEffect
    } catch (err: any) {
      console.error("Error creating pool:", err)
      setErrors({ ...errors, agreeToTerms: err.message || "Failed to create pool. Please try again." })
    }
  }

  // Handle successful transaction confirmation and get pool ID
  useEffect(() => {
    if (isConfirmed && receipt && isConnected) {
      try {
        // Parse the PotCreated event from transaction logs
        // Event signature: PotCreated(uint256 indexed potId, address indexed creator, string name, uint256 totalCycles, uint256 contributionAmount)
        if (receipt.logs && receipt.logs.length > 0) {
          // Find the PotCreated event
          const potCreatedEvent = receipt.logs.find((log: any) => {
            // PotCreated event topic[0] would be keccak256("PotCreated(uint256,address,string,uint256,uint256)")
            return log.topics && log.topics.length >= 2
          })
          
          if (potCreatedEvent && potCreatedEvent.topics) {
            // The first indexed parameter (potId) is in topics[1]
            const potId = BigInt(potCreatedEvent.topics[1])
            router.push(`/pools/${potId}`)
            return
          }
        }
        
        // Fallback: use total pots to get the new pool ID (will be totalPots - 1 after increment)
        if (totalPotsBefore) {
          const newPotId = Number(totalPotsBefore)
          router.push(`/pools/${newPotId}`)
          return
        }
        
        // If we can't find the pool ID, redirect to pools list
        router.push("/pools")
      } catch (err) {
  ...
  }, [isConfirmed, receipt, totalPotsBefore, isConnected, router])
